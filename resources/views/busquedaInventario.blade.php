...
<div>
    <div align="center">
        <h3> Datos de Busqueda de Productos</h3>
        </div>
    ...
    <div class="card-header">
        <ul class="nav nav-tabs card-header-tabs" style="background-color: #FFFEE4;">
            ...
            <li  class="nav-item ml-auto">
                <form class="d-flex" action="{{ route('refrescarInventario') }}" method="post" >
                    @csrf
                    <input class="form-control me-2" type="search" placeholder="Codigo  o Nombre" aria-label="Search" name="nombre" id="nombre">
...
<tbody>
@foreach($registroDe as $inventarioss)
    <tr>
    
    <td>{{$inventarioss->id}}</td>
    ...
    <td>{{$inventarioss->iva}}</td>
        ...
        <form action="{{ route('eliminarCategoria',$inventarioss->id)}}" method="POST">
                        @csrf
